...
class SequenceModel(SequenceModule):
    def __init__(
        ...
        n_apply=1,         # Number of times to apply each block of layers in model (for layer tied models)
    ):
        super().__init__()
        ...
        self.layer_batch_size = 1  # batching multiple layers together to re-applying, e.g attention + FF
        self.n_apply = n_apply  # number of times to apply each layer batch
        ...
        self.layer_batch_size = len(layer)
    ...
    def forward(self, inputs, *args, state=None, **kwargs):
        ...
        if self.n_apply == 1:
            ...
            assert state is None, "State must be None for layer tied models"
            b = self.layer_batch_size
            layer_batches = [self.layers[i * b: (i + 1) * b] for i in range(len(self.layers) // b)]
            for layer_batch in layer_batches:
                # apply each layer batch n_apply times
                for _ in range(self.n_apply):
                    for layer in layer_batch:
                        outputs, state = layer(outputs, *args, state=None, **kwargs)
                        next_states.append(state)
                        if self.track_norms: output_norms.append(torch.mean(outputs.detach() ** 2))
