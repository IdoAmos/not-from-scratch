...
class MaskedSequenceDatasetWrapper(torch.utils.data.Dataset):
    """
    Wraps an existing dataset to apply masked pre-training.

    Parameters:
        dataset (torch.utils.data.Dataset): The original dataset to be masked.
        mlm_prob (float, optional): Probability of masking a token. Default is 0.0.
        causal_lm (bool, optional): Whether to apply causal masking. Default is False.
        lm_loss (str, optional): Loss function to use for masked tokens. Can be 'l1' or 'ce'. Default is 'l1'.
        ignore_val (bool, optional): Whether to ignore the loss for non-masked tokens. Default is True.

    Note:
        - Transforms are supplied separately for the case of using Cross Entropy loss.
        - Labels should match source tokens and not transformed tokens that might differ per image.
    """
    def __init__(self, dataset, mlm_prob=0.0, causal_lm=False, lm_loss='l1', ignore_val=True,
                 span_masking=False, span_length=0):
        if causal_lm:
            assert mlm_prob ==0.0, "Causal LM not supported with MLM"

        self.dataset = dataset  # the original dataset to be masked
        self.mlm_prob = mlm_prob  # probability of masking a token
        self.causal_lm = causal_lm  # whether to apply causal masking
        self.lm_loss = lm_loss  # loss function to use for masked tokens
        self.span_masking = span_masking
        self.span_length = span_length

        if ignore_val:  # whether to ignore the loss for non-masked tokens
            self.ignore_non_masked = True
            self.ignore_val = -100 if lm_loss == 'ce' else -10000.0
        else:
            self.ignore_non_masked = False
    ...
    def __getitem__(self, index):
        """
        This method needs to be implemented per dataset.

        Args:
            index (int): Index of the sample to retrieve from the original dataset.

        Returns:
            Any: The item at the specified index from the original dataset.
        """
        pass
    ...
    def mask(self, sample, sample_classes=None, allow_features=False):
        """
        ...
        Args:
            ...
            allow_features (bool, optional): Whether to allow features in the sample. Use with caution
        Returns:
        ...
        if not allow_features:
            assert sample.ndim == 2 and sample.shape[-1] == 1, "masked dataset should use scalar sequences"  # [L 1]
