...
class MaskedInformer(MaskedSequenceDatasetWrapper):
    ...
    def mask(self, sample, sample_classes=None):
        ...
        assert not lm_loss == 'ce', "Informer-LM tasks do not support CE"
    ...
    def __getitem__(self, item):
        data_sample = self.dataset[item]
        sample = data_sample[0]
        sample, target = self.mask(sample)

        return sample, target, data_sample[-2], data_sample[-1]


class InformerSequenceDatasetLM(SequenceDataset):
    ...
    def setup(self):
        self.data_dir = self.data_dir or default_data_path / 'informer' / self._name_[:-3]  # drop the '_lm' suffix
        ...
        self.n_tokens_time = self.dataset_train.n_tokens_time
        self.d_input = self.dataset_train.d_input
        self.d_output = self.dataset_train.d_output
        self.l_output = None

        self.dataset_train = MaskedInformer(self.dataset_train, mlm_prob=self.mlm_prob, causal_lm=self.causal_lm,
                                            lm_loss=self.lm_loss, ignore_val=self.ignore_val,
                                            span_masking=self.span_masking, span_length=self.span_length)
        self.dataset_val = MaskedInformer(self.dataset_val, mlm_prob=self.mlm_prob, causal_lm=self.causal_lm,
                                          lm_loss=self.lm_loss, ignore_val=self.ignore_val,
                                          span_masking=self.span_masking, span_length=self.span_length)
        self.dataset_test = MaskedInformer(self.dataset_test, mlm_prob=self.mlm_prob, causal_lm=self.causal_lm,
                                           lm_loss=self.lm_loss, ignore_val=self.ignore_val,
                                           span_masking=self.span_masking, span_length=self.span_length)


class ETTHourLM(InformerSequenceDatasetLM):
    _name_ = "etth_lm"
    ...
    init_defaults = {
        ...
        "causal_lm": True,  # self pre-training : predict next pixel in sequence
...
class ETTMinuteLM(InformerSequenceDataset):
    _name_ = "ettm_lm"
...
class WeatherLM(InformerSequenceDataset):
    _name_ = "weather"
...
class ECLLM(InformerSequenceDataset):
    _name_ = "ecl"
