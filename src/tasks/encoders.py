...
# Added by Ido Amos 04/06/2023
from rotary_embedding_torch import RotaryEmbedding
from src.models.sequence.ss import s4
...
class MaskedEncoder(Encoder):
    ...
    def __init__(self, d_model, p=0.15, causal=False, span_masking=False):
        super(MaskedEncoder).__init__()
        self.p = p
        ...
        if causal:
            raise NotImplementedError("Causal masking not implemented in MaskedEncoder")
        if span_masking:
            raise NotImplementedError("Span masking not implemented in MaskedEncoder")
    def forward(self, x, **kwargs):
        return self.random_mask(x, **kwargs)

    def random_mask(self, x, mask_positions=None, **kwargs):
        assert x.dim() == 3
        assert x.size(-1) == self.d_model

        if mask_positions is not None:
            assert mask_positions.shape == x.shape[:-1]
            x[mask_positions, :] = self.mask_token
        else:

            mask = torch.rand(x.size(0), x.size(1)) < self.p
...
# Adapted from https://github.com/lucidrains/rotary-embedding-torch/tree/main
class RotaryPositionalEncoder(Encoder):
    r"""Inject some information about the relative or absolute position of the tokens
            in the sequence. The positional encodings have the same dimension as
    ...
    def forward(self, x):
        out = self.rotary_emb.rotate_queries_or_keys(x)
        return out


class SSPositionalEncoder(Encoder):
    r"""Inject some information about the relative or absolute position of the tokens
            in the sequence. The positional encodings have the same dimension as
        ...
        """
    def __init__(self, d_model, dropout=0.0, trainable=False, linear=False, bidirectional=False, dataless=False):
        super().__init__()
        self.state_size = 64
        self.ss = s4.S4(d_model=d_model, d_state=self.state_size, dropout=dropout, linear=linear,
                        bidirectional=bidirectional, transposed=False)
        self.norm = nn.LayerNorm(d_model)

        # if dataless, then the root is a learnable parameter
        if dataless:
            self.root = nn.Parameter(torch.randn(1, d_model))
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'root'):
            # expand root to be a sequence of the same length as x
            pos = repeat(self.root, '1 d -> b l d', b=x.shape[0], l=x.shape[1])
            y, _ = self.ss(pos)
        else:
            y, _ = self.ss(x)

        x = self.norm(y) + self.norm(x)

        return x


class AppendCLS(Encoder):
    """
    Append CLS token to input at last position
    """
    def __init__(self, d_model, last=False):
        super().__init__()
        self.d_model = d_model
    ...
    def forward(self, x):
        ...
        Args:
            x: (B, L, d_model)
        """
        cls = repeat(self.cls, '1 d -> b l d', b=x.shape[0], l=1)
...
registry = {
    ...
    "rotary": RotaryPositionalEncoder,
    "ss_position": SSPositionalEncoder,
    "cls": AppendCLS,
