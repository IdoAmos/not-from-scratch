...
class SequenceDecoder(Decoder):
    ...
    def forward(self, x, state=None, lengths=None, l_output=None):
        ...
        if self.mode == "last":
            ...
            restrict= lambda x: torch.cummax(x, dim=-2)[0][..., -l_output:, :]
...
class NoneLinearSequenceDecoder(Decoder):
    """
    a none linear decoder that applies a 2-layer MLP to the pooled input
    """
    def __init__(
        self, d_model, d_output=None, l_output=None, use_lengths=False, mode="last", type='mlp'):
        super().__init__()
        ...
        elif type == 'none' or type == 'id':
            self.output_transform = nn.Identity()
        else:
            raise NotImplementedError(f"Unknown type {type} for NoneLinearSequenceDecoder")
...
class CLSDecoder(Decoder):
    def __init__(self, d_model, d_output=None, mode="last", type='mlp'):
        super().__init__()

        d_output = d_model if d_output is None else d_output
        self.type = type
        self.mode = mode
        self.cls_pos = -1 if mode == 'last' else 0
...
registry = {
    ...
    "sequence_nonelinear": NoneLinearSequenceDecoder,
    "cls": CLSDecoder,
    "nd": NDDecoder,
...
dataset_attrs = {
    ...
    "sequence_nonelinear": ["d_output", "l_output"],
    "cls": ["d_output"],
    "nd": ["d_output"],
    "retrieval": ["d_output"],
    "state": ["d_output"],
